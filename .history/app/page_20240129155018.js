...
export default function Home() {
  ...
  const handleChange = (selectedOption) => {
    ...
    setSelectedProperty(null); // Reset selected property when the main category changes
    setOtherValue(""); // Reset other value when the main category changes
  };

  const handleChildChange = (selectedChildOption) => {
    setSelectedChild(selectedChildOption);
    setSelectedProperty(null); // Reset selected property when the sub-category changes
    setOtherValue(""); // Reset other value when the sub-category changes
  };
  ...
  const handlePropertyChange = (selectedPropertyOption) => {
    ...
    if (selectedPropertyOption.label !== "other") {
      setOtherValue(""); // Reset other value if the user selects any option other than "other"
    }
  };
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      {selectedCat && (
        <>
          ...
          {selectedChild && (
            <h2>Selected Sub-Category: {selectedChild.label}</h2>
          )}
          {selectedProperty && (
            <h2>
              Selected Property:{" "}
              {selectedProperty.label === "other"
                ? "Other"
