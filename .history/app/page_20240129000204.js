...
export default function Home() {
  ...
  const handleChange = async selectedOption => {
    // Fetch the selected category's children
    const categoryId = selectedOption.value;
    const response = await fetch(`https://staging.mazaady.com/api/v1/get_category_details/${categoryId}`);
    const result = await response.json();
  
    // Update state with selected category and its children
    setSelectedCat({
      value: categoryId,
